...
//Showing peoples to add

router.get('/recommended_peoples_to_add',auth,asyncmiddleware(async(req,res)=>{
    const token = req.header('x-auth-token');
    const decoded = jwt.verify(token, config.get('JwtPrivatekey'));
    if (!token){
        return res.status(401).send('UnAuth');
...
//Adding friend
router.put('/Add_friend/:id',auth,asyncmiddleware(async(req,res)=>{
    const token = req.header('x-auth-token');
    const decoded = jwt.verify(token, config.get('JwtPrivatekey'));
    if (!token){
        return res.status(401).send('UnAuth');
     ...
     await User.findByIdAndUpdate(decoded.id,{$push:{
        addedFriend:req.params.id
    }});

    await User.findByIdAndUpdate(req.params.id,{$push:{
        friendRequest:decoded.id
    }});
...
//Confirm Request
router.put('/Confirm_request/:id',auth,asyncmiddleware(async(req,res)=>{
    const token = req.header('x-auth-token');
    const decoded = jwt.verify(token, config.get('JwtPrivatekey'));
    if (!token){
        return res.status(401).send('UnAuth');
    ...
    await User.findByIdAndUpdate(req.params.id,{$pull:{
        addedFriend:decoded.id
    }});

    await User.findByIdAndUpdate(decoded.id,{$pull:{
